...
class mailEclipse
{
    ...
    static public function getViewNamespace()
    {
        return config('maileclipse.view.namespace', 'mail');
    }
    ...
    static public function updateTemplate($request)
    {
        ...
        if (!is_null($template)) {
            ...
            $template_view = self::getViewNamespace() . '::' . $template->template_slug;
            $template_plaintext_view = $template_view . '_plain_text';
        ...
        if (!is_null($template)) {
            ...
            if (View::exists($template_view)) {
                ...
                $templateData = collect([
                    ...
                    'view' => self::getViewNamespace() . '::' . $template->template_slug,
    ...
    static public function createTemplate($request)
    {
        ...
        $view = mailEclipse::getViewNamespace() . '::' . $request->template_name;
        ...
        if (!view()->exists($view) && !DB::table(self::$templates_table)->where('template_slug', '=', $templatename)->exists()) {
            ...
            $dir = config('maileclipse.view_dir', resource_path('views/mail'));
    ...
    static public function previewMarkdownViewContent($simpleview = false, $content, $viewName, $template = false, $namespace = null)
    {
        ...
        if (file_exists($dir)) {
            ...
            return self::renderPreview($simpleview, self::getViewNamespace() . '::draft.' . $viewName, $template, $instance);
        }
        ...
        try {
            ...
            if ($template) {
				...
				<h5 class="alert-heading">Notice:</h5>
				 <p>You can\'t add variables within a template editor. because they are undefined until you bind template with a mailable that actually has data.</p>
	    	</div>';
